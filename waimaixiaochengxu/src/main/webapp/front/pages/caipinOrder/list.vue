				...
				<view class="bg-white margin-sm padding-sm solids-bottom radius text-content">
					...
					<view class="bg-white margin-xs padding-xs  radius text-content">
						...
						<view class="flex justify-end"
							v-if="item.caipinOrderTypes == 3 && item.caipinOrderCourierName != null">
							{{ item.caipinOrderCourierName }}: <text class="text-bold solids-bottom">
								{{ item.caipinOrderCourierNumber }}</text></view>
					...
					<view class="flex justify-end" style="font-size: 24upx;">
						...
						<view v-if="item.caipinOrderTypes == 1 && item.shangjia == user.id">
							<view @tap="openDeliver(item.id)" class="round cu-btn lines-grey mid  margin-right-sm">发货
							</view>
						</view>
						<view v-if="item.caipinOrderTypes == 3">
							<view @tap="receiving(item.id)" class="round cu-btn lines-grey mid  margin-right-sm">取餐
							</view>
						</view>
					...
					<view class="uni-flex uni-column"
						...
						<view class="flex-item flex-item-V">
							...
							<textarea
								...
								v-model="caipinCommentbackText" placeholder="评论内容"></textarea>
						</view>
			...
			<uni-popup class="popup-content" ref="deliver" type="center">
				<form class="popup-form">
					<view class="uni-flex uni-column"
						...
						<view class="flex-item flex-item-V"
							style="text-align: right;background: #FFFFFF;margin-top: 10rpx;">
...
<script>
	export default {
		...
		methods: {
			...
			async deliver(id) {
				let _this = this;
				if (_this.caipinOrderCourierName == null || _this.caipinOrderCourierName == "" || _this
					.caipinOrderCourierName == "null") {
					return _this.$utils.msg('配送员不能为空');
				}
				if (_this.caipinOrderCourierNumber == null || _this.caipinOrderCourierNumber == "" || _this
					.caipinOrderCourierNumber == "null") {
